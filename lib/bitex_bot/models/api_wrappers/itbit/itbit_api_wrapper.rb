...
class ItbitApiWrapper < ApiWrapper
  ...
  def send_order(type, price, quantity)
    ...
    BitexBot::Robot.log(:error, :wrapper, :placement, 'Captured Timeout on itbit')
    latest = last_order_by(price)
    return latest if latest.present?

    BitexBot::Robot.log(:error, :wrapper, :placement, 'Could not find my order')
    raise e
  end
  ...
  def rounded_price(trade_type, price)
    if trade_type == :buy
      price - price % 0.25
    else
      price + price % 0.25
