module BitexBot
  ...
  class SellOpeningFlow < OpeningFlow
    class << self
      ...
      #
      def create_for_market(usd_balance, order_book, transactions, bitex_fee, other_fee, store)
        super
      end
      ...
      def safest_price(transactions, order_book, bitcoins_to_use)
        OrderBookSimulator.run(Settings.time_to_live, transactions, order_book, nil, bitcoins_to_use)
      end

      def remote_value_to_use(value_to_use_needed, safest_price)
      ...
      def bitex_price(btc_to_sell, usd_to_spend_re_buying)
