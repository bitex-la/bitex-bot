  ...
  module ApiWrappers
    ...
    class Itbit < Base
      ...
      def amount_and_quantity(order_id)
        order = client::Order.find(order_id)
      ...
      def orders
        client::Order.all(instrument: currency_pair[:name], status: :open).map { |o| order_parser(o) }
      end
      ...
      def wallet
        client::Wallet.all.find { |w| w[:id] == client.default_wallet_id }
      end
      ...
      def last_order_by(price)
        client::Order.all.select { |o| o.price == price && (o.created_time - Time.now.to_i).abs < 500 }.first
      end
