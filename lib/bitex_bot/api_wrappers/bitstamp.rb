...
module BitexBot
  module ApiWrappers
    class Bitstamp < Base
      ...
      def balance
        balance_summary_parser(client.balance(currency_pair[:name]).symbolize_keys)
      ...
      def orders
        client.orders.all(currency_pair: currency_pair[:name]).map { |o| order_parser(o) }
      rescue StandardError => e
        raise ApiWrapperError, "Bitstamp orders failed: #{e.message}"
      end

      def send_order(type, price, quantity)
        order = client.orders.send(type, currency_pair: currency_pair[:name], amount: quantity.round(4), price: price.round(2))
        order_parser(order) unless order.error.present?
      end

      def transactions
        client.transactions(currency_pair[:name]).map { |t| transaction_parser(t) }
      rescue StandardError => e
        raise ApiWrapperError, "Bitstamp transactions failed: #{e.message}"
      end

      def user_transactions
        client.user_transactions.all(currency_pair: currency_pair[:name]).map { |ut| user_transaction_parser(ut) }
      rescue StandardError => e
