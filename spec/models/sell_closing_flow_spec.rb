...
describe BitexBot::SellClosingFlow do
  ...
  describe 'has a valid factory' do
    subject(:flow) { create(:sell_closing_flow) }
  ...
  describe '.close_market' do
    ...
    context 'with open positions' do
      before(:each) do
        ...
        allow(BitexBot::Robot).to receive_message_chain(:taker, :enough_order_size?).with(10, 2, :buy).and_return(enough_order_size)
      end
      ...
      context 'enough order size for taker' do
        ...
        before(:each) do
          ...
          allow(BitexBot::Robot).to receive_message_chain(:taker, :place_order).with(:buy, 2, 10).and_return(BitexBot::Exchanges::Order.new('65'))
      ...
      context 'with close positions' do
        ...
        context 'and active position' do
          ...
          context 'and not enough order size' do
            before(:each) do
              allow(BitexBot::Robot).to receive_message_chain(:taker, :enough_order_size?).with(10, 20, :buy).and_return(false)
          ...
          context 'and enough order size' do
            before(:each) do
              allow(BitexBot::Robot).to receive_message_chain(:taker, :enough_order_size?).with(10, 20, :buy).and_return(true)
  ...
  describe '#earning_summary' do
    ...
    its(:earning_summary) do
      is_expected.to eq("BitexBot::SellClosingFlow ##{flow.id} earned: fiat profit: 10.0, crypto profit: 1.0.")
    end
  end
