...
describe BitexBot::SellOpeningFlow do
  ...
  describe '.sought_transaction' do
    ...
    context 'non sought by' do
      ...
      before(:each) do
        ...
        create(:open_sell, transaction_id: trade.order_id)
      end
  ...
  describe '#price_scale' do
    subject(:flow) { create(:sell_opening_flow, price: 100, value_to_use: 200) }

    it { expect(flow.price_scale(0.1)).to eq(110) }
    it { expect(flow.price_scale(0.3)).to eq(130) }
  end
  ...
  describe '#place_orders' do
    ...
    subject(:flow) { create(:sell_opening_flow, price: 100, value_to_use: 100) }

    it 'place 5 orders' do
      expect { flow.place_orders }.to change { BitexBot::OpeningAsk.count }.by(5)
    end
    ...
    context 'opening orders depth' do
      ...
      it_behaves_like 'deepnes for', 2, 'second_tip', 101, 25
      it_behaves_like 'deepnes for', 3, 'support', 102, 5
      it_behaves_like 'deepnes for', 4, 'informant', 105, 15
      it_behaves_like 'deepnes for', 5, 'final', 110, 5
    end
  end
  ...
  describe '#summary' do
    before(:each) do
      allow(BitexBot::Robot).to receive_message_chain(:maker, :base).and_return('MAKER_BASE')
      allow(BitexBot::Robot).to receive_message_chain(:maker, :quote).and_return('MAKER_QUOTE')
      allow(BitexBot::Settings).to receive(:selling_fx_rate).and_return(100.to_d)
    end
    ...
    its(:summary) do
      is_expected.to eq([
        'Sell flow #1: order_id: 1, role: first_tip, status: executing, price: MAKER_BASE 1000.0, amount: MAKER_QUOTE 15.0.',
        'Sell flow #1: order_id: 2, role: first_tip, status: settling, price: MAKER_BASE 2000.0, amount: MAKER_QUOTE 25.0.'
      ])
    end
  end
