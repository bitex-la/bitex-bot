...
describe BitexBot::OpeningAsk do
  ...
  describe '#finalise' do
    ...
    context 'with executing status' do
      ...
      let(:order) { BitexBot::Exchanges::Order.new('1', :ask, 400, 2, Time.now.to_i, order_status, 'client_order_id', double) }
  ...
  describe '#summary' do
    before(:each) do
      allow(BitexBot::SellOpeningFlow).to receive(:fx_rate).and_return(10.to_d)
    ...
    subject(:opening_order) { create(:opening_ask, order_id: 'order#45', price: 300, amount: 3) }

    its(:summary) do
      is_expected.to eq(
        'Sell flow #1: order_id: order#45, role: first_tip, status: executing, price: MAKER_BASE 3000.0, amount: MAKER_QUOTE 3.0.'
      )
    end
  end
  ...
  describe '#order' do
    ...
    let(:order) { BitexBot::Exchanges::Order.new('1', :ask, 400, 2, Time.now.to_i, :status, 'client_order_id', double) }
